...
export default function Navbar() {
  ...
  return (
    ...
    >
          ...
          {navLinks.map((link) => (
            ...
            >
              ...
              {link.hasDropdown && activeDropdown === link.name && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-[600px] bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden grid grid-cols-2 p-4 gap-2 animate-in fade-in slide-in-from-top-2 duration-200">
                  {link.items?.map((item) => {
      ...
      {/* Mobile Navigation */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-slate-900 border-b border-white/10 overflow-hidden animate-in slide-in-from-top-2 duration-200">
          <div className="px-6 py-4 flex flex-col gap-4">
            {navLinks.map((link) => (
              <div key={link.name} className="flex flex-col">
                ...
                {/* Mobile Dropdown */}
                {link.hasDropdown && activeDropdown === link.name && (
                  <div className="flex flex-col gap-3 pl-4 mt-3 border-l border-white/10 animate-in slide-in-from-top-2 duration-200">
                    {link.items?.map((item) => (
                      <Link
                        key={item.slug}
                        href={`${link.href}/${item.slug}`}
